...
def main():
    ...
    glass_image='glass3.png'
    mouse_image='hand2.jpg'
    running=True
    #背景音乐
    pygame.mixer.music.load('B1.ogg')
    pygame.mixer.music.play()
    #音效
    loser_sound=pygame.mixer.Sound('what.wav')
    laugh_sound=pygame.mixer.Sound('evil.wav')
    winner_sound=pygame.mixer.Sound('laughZ.wav')
    hole_sound=pygame.mixer.Sound('laughIN.wav')
    #音乐播放完时游戏结束
    GAMEOVER=USEREVENT
    ...
    for i in range(5):
        ...
        ball=Ball(gray_image,green_image,position,speed,bg_size,5*(i+1))
    ...
    while running:
                    ...
                    for each in group:
                        if each.control:
                            ...
                            if not hole:
                                ...
                                msg=pygame.image.load('msg4.png').convert_alpha()
                                msg_pos=(width-msg.get_width())//2,(height-msg.get_height())//2-200
                                msgs.append((msg,msg_pos))
                                msg=pygame.image.load('msg5.png').convert_alpha()
                                msg_pos=(width-msg.get_width())//2,(height-msg.get_height())//2
                                msgs.append((msg,msg_pos))
                                msg=pygame.image.load('msg6.png').convert_alpha()
                                msg_pos=(width-msg.get_width())//2,(height-msg.get_height())//2+200
                                msgs.append((msg,msg_pos))
                                laugh_sound.play()
        ...
        for each in balls:
            ...
            if each.collide:
                ...
                each.collide=False
            if each.control:
                screen.blit(each.greenball_image,each.rect)    #变绿色小球
            else:
        ...
        for i in hole:
            pygame.draw.circle(screen,(0,255,0),(i[0]+52,i[2]+52),50,1)
